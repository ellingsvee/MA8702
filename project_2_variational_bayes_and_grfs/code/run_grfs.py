...
if __name__ == "__main__":
    ...
    s = generate_grid(N=100)

    x = sample_centered_grf(
        sample_key,
        s,
    ...
    alpha = 1.0
    x_full = x + mean_function(s) * alpha

    plot_sample(s, x_full, filename=PLT_PATH / "grf_full_field.svg", heatmap=True)

    s_obs, A = random_observation_matrix(obs_key, s, N_obs=200)
    y_obs = A @ x_full + jr.normal(noise_key, (200,)) * 0.05

    plot_sample(s_obs, y_obs, filename=PLT_PATH / "grf_observations.svg", heatmap=False)
