...
def main():
    parser = argparse.ArgumentParser(
        description="Run multivariate VI vs MCMC benchmarks"
    )
    parser.add_argument(
        "--gpu", action="store_true", dest="with_gpu", help="Enable GPU benchmarks"
    )
    args = parser.parse_args()

    output.mkdir(exist_ok=True)

    has_gpu = bool(jax.devices("gpu")) and args.with_gpu
    ...
    if GENERATE_PLOTS:
        plot_device = "GPU" if has_gpu and args.with_gpu else "CPU"
